...
df = spark.read.json('hdfs://namenode:9000/forex/forex_currencies.json')

# Drop the duplicated rows based on the base and last_update columns
forex_rates = df.select('base', 'date', 'rates.usd', 'rates.cad', 'rates.gbp', 'rates.jpy', 'rates.nzd') \
    .fillna(0, subset=['USD', 'JPY', 'CAD', 'GBP', 'NZD'])
